<script lang="ts">
    import type { AnalysisResultItem, AnalysisResults } from '$lib/SlopClassifier';
...
<div>
    <h3 class="text-lg font-semibold text-gray-800 mb-4">Token-by-Token Analysis</h3>

    <!-- Legend -->
    <div class="flex justify-between mb-3 text-xs text-gray-600 px-1">
        <div class="flex items-center">
            <span class="w-4 h-2.5 rounded-sm mr-1.5" style="background-color: #ff6b6b;"></span> Unexpected
        </div>
        <div class="flex items-center">
            <span class="w-4 h-2.5 rounded-sm mr-1.5" style="background-color: #ffe66d;"></span> Average
        </div>
        <div class="flex items-center">
            <span class="w-4 h-2.5 rounded-sm mr-1.5" style="background-color: #4ecdc4;"></span> Expected
        </div>
    </div>
    ...
    {#if tokens.length > 0 && results.length === 0}
        <p class="text-sm text-gray-500 mt-4 px-1">
            Only one token was provided. Cannot calculate predictability for the first token.
        </p>
    {/if}
</div>
