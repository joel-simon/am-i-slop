...
export class SlopAnalyzer {
    ...
    private onTokenProcessedCallback?: (item: AnalysisResultItem | string) => void; // Can be a single token string (first token) or an item
    ...
    private async _loadModel(modelName: string): Promise<void> {
        ...
        try {
            ...
            this.tokenizer = await AutoTokenizer.from_pretrained(`Xenova/${modelName}`);
            this.updateProgressCallback(0.4, `Loading model weights for ${modelName}...`);
            this.model = await AutoModelForCausalLM.from_pretrained(`Xenova/${modelName}`);
    ...
    public async analyzeText(text: string, modelName: string): Promise<AnalysisResults> {
        this.setErrorMessageCallback(null);

        try {
            if (!this.model || this.currentModelName !== modelName || !this.tokenizer) {
            ...
            for (let i = 0; i < tokenIds.length - 1; i++) {
                ...
                const logProbability = probability > 0 ? Math.log(probability) : -Infinity;
            ...
            const avgLog = logs.length ? logs.reduce((a, b) => a + b, 0) / logs.length : -Infinity;
            ...
            if (err.message) {
                ...
                this.setErrorMessageCallback('An unexpected error occurred during analysis.');
            }
